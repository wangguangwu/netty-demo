    ...
    private static void connect(Bootstrap bootstrap, String host, int port, int retry) {
        bootstrap.connect(host, port).addListener(future -> {
            if (future.isSuccess()) {
                ...
                log.error("retry times is exhausted, give up connect");
            } else {
                int order = (MAX_RETRY - retry) + 1;
                int delay = 1 << order;
                log.error("{}: connect failed. the {} times to reconnect", new Date(), order);
                bootstrap.config()
                        .group()
        ...
        new Thread(() -> {
            while (!Thread.interrupted()) {
                if (LoginUtil.hasLogin(channel)) {
                    log.info("please write message send to server:");
